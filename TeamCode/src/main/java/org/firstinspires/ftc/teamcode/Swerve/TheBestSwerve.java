...
public class TheBestSwerve {
  ...
  private boolean get_drive_direction(
      double steering_angle, boolean current_drive_direction, double schmitt_breakpoint_tolerance) {
    ...
    if (steer_direction == SteerDirection.left) {
      ...
      // Check if steering_angle is below the bottom threshold to result in a shift to drive
      // backwards
  ...
  /**
   * Determines the general direction of the robot strafe. General in this case is either up, down,
   * left, or right. This function is used to determine which two wheels should be the drive wheels
   * during a turn.
   */
...
//    double robot_theta = odometry.getHeading().getRadians();
    ...
    if (joy_magnitude < 0.01) steering_angle = 0.5;

    //		telemetry.addLine("Servo steering: " + steering_angle);

    /**
     * At this point, we have vector magnitudes and angles associated with both the joystick input
     * and the robot's odometry. The robot's odometry operates on the unit circle, meaning its
     * vector magnitude is irrelevant for determining field-centric control. This is why simply
     * computing the vector sum is insufficient for accurate field-centric steering adjustments.
     *
     * <p>The `steering_angle` variable represents the actual orientation of the swerve module
     * wheels required for field-centric strafing. However, a key challenge arises because the
     * swerve drive modules have a limited steering range—typically 270 degrees. If the wheels
     * attempt to rotate past this limit to align with the intended direction, they will suddenly
     * rotate the full 270 degrees to reach the target angle from the opposite side.
     *
     * <p>One way to address this issue is to constrain steering adjustments to a 180-degree range.
     * If the computed angle exceeds this threshold, the direction should be inverted, and the drive
     * motor should reverse. This method works effectively as long as the robot isn't continuously
     * strafing left and right. Otherwise, a hysteresis effect occurs, causing oscillations when the
     * wheels attempt to realign near the 180-degree boundary.
     *
     * <p>To mitigate this, a software-based Schmitt trigger can be implemented. By defining upper
     * and lower threshold limits, the system can establish a stable directional state, reducing
     * unwanted oscillations. The `steering_angle` variable should be used to determine transitions
     * between positive and negative directions to maintain smooth and predictable steering
     * behavior.
     *
     * <p>The `steering_angle` variable represents the steering direction relative to the robot's
     * frame. The values are mapped as follows:
     *
     * <p>Direction | steering_angle Value ------------------------------------ Forward | 0.5 Left |
     * 0.75 Right | 0.25 Reverse | 0 or 1
     */
    boolean previous_drive_direction = drive_direction;
    drive_direction =
        get_drive_direction(steering_angle, drive_direction, schmitt_breakpoint_tolerance);
