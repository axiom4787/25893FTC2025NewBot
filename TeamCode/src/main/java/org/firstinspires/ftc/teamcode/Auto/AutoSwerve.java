...
public class AutoSwerve {
  ...
  // odo.getPosX is forward on robot                                             // why is this
  // here?
  public void setMotors(double pwr, double z) {
    // because br can't rotate to the right position to go straight
    if (z == 1) motorBR.setPower(pwr);
    else motorBR.setPower(pwr);
    motorBL.setPower(pwr);
    motorFL.setPower(pwr);
    motorFR.setPower(pwr);
  }

  public double set_Servo_Angle(
      AnalogInput analogInput, Servo servo, double desired_normalized_angle) {
    ...
    if (delta_to_reference > 0.09) servo_speed = delta_to_reference;
    if (delta_to_reference > .18) servo_speed = .25;
    if (delta_to_reference > .25) servo_speed = .3;

    double tolerance = 0.01;

    if (delta_to_reference < (-1 * tolerance)) servo.setPosition(0.5 + servo_speed);
    else if (delta_to_reference > tolerance) servo.setPosition(0.5 - servo_speed);
    else {
